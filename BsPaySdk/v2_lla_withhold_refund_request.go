...
type V2LlaWithholdRefundRequest struct {
...
func (bp *BsPay) StrV2LlaWithholdRefundRequest(reqStr string) (map[string]interface{}, error) {
	reqParam := V2LlaWithholdRefundRequest{}
	json.Unmarshal([]byte(reqStr), &reqParam)
    json.Unmarshal([]byte(reqStr), &reqParam.ExtendInfos)
	return bp.V2LlaWithholdRefundRequest(reqParam)
}

func (bp *BsPay) V2LlaWithholdRefundRequest(reqParam V2LlaWithholdRefundRequest) (map[string]interface{}, error) {
	var url = BASE_API_TEST_URL_V2
	if bp.IsProdMode {
		url = BASE_API_URL_V2
	}
	reqUrl := url + V2_LLA_WITHHOLD_REFUND
